...
const InstallationPage = () => {
  ...
  data.installations.nodes = data.installations.nodes.filter(
    (node: { remoteId: any }) => specialIds.includes(node.remoteId) == false
  );
  ...
  const entryThree = Math.floor(
    Math.random() * data.installations.nodes.length
  );
  const entryFour = Math.floor(Math.random() * data.installations.nodes.length);

  const preview = [
    data.installations.nodes[entryOne],
    data.installations.nodes[entryTwo],
    data.installations.nodes[entryThree],
    data.installations.nodes[entryFour]
  ];
